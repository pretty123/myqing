...
class WxwallModuleProcessor extends WeModuleProcessor {
	...
	private function post() {
		...
		if (empty($member['nickname']) || empty($member['avatar'])) {
			return $this->respText('发表话题前请<a target="_blank" href="'.$this->buildSiteUrl($this->createMobileUrl('register')).'">登记</a>您的信息。');
		}
		...
		if ($this->message['type'] == 'image') {
			...
			$filename = IA_ROOT . '/addons/wxwall/template/image/' . $member['rid'] . '/' . random(30) . '.jpg';
