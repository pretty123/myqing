...
<!-- 我的会员卡 -->
<?php  if($do == 'mycard') { ?>
	<script src="<?php  echo $_W['siteroot'];?>app/resource/js/app/common.js"></script>
		...
		<form name="theform" method="post" role="form" id="form1">
			...
			<div class="tab-content">
				<div class="tab-pane fade in active" id="email">
					<?php  if(!empty($reregister)) { ?>
					<div class="form-group">
						<input type="text" name="<?php  if($item == 'email') { ?>email<?php  } else { ?>mobile<?php  } ?>" value="" class="form-control input-lg" placeholder="<?php  if($item == 'email') { ?>E-Mail地址<?php  } else { ?>手机号码<?php  } ?>">
					</div>
					<?php  } ?>
					<div class="form-group">
