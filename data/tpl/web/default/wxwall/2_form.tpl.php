<?php defined('IN_IA') or exit('Access Denied');?><div class="panel panel-default">
	...
	<div class="panel-body">
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<select name="acid" class="form-control" >
					...
					<option value="<?php  echo $account['acid'];?>" <?php  if($reply['acid']==$account['acid']) { ?>selected="selected"<?php  } ?>><?php  echo $account['name'];?></option>
					<?php  } } ?>
				</select>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<input type="text" value="<?php  echo $reply['quit_command'];?>" class="form-control" name="quit-command">
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<textarea style="height:200px;" class="form-control" name="enter-tips" id="enter-tips" cols="70"><?php  echo $reply['enter_tips'];?></textarea>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<textarea style="height:200px;" class="form-control" name="send-tips" id="send-tips" cols="70"><?php  echo $reply['send_tips'];?></textarea>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<textarea style="height:200px;" class="form-control" name="quit-tips" id="quit-tips" cols="70"><?php  echo $reply['quit_tips'];?></textarea>
		...
		<div class="form-group">
			...
			<div class="col-sm-9">
				<label class="radio-inline">
					<input type="radio" name="walls[tx][status]" value="1" id="syncwall_tx_1" <?php  if(!empty($reply['syncwall']['tx']['status'])) { ?>checked="true"<?php  } ?> onclick="$('#syncwall_tx').show();"> 是
				</label>
				<label class="radio-inline">
					<input type="radio" name="walls[tx][status]" value="0" id="syncwall_tx_0" <?php  if(empty($reply['syncwall']['tx']['status'])) { ?>checked="true"<?php  } ?> onclick="$('#syncwall_tx').hide();"> 否
				</label>
				<span class="help-block">开启此选项后，系统自动获取指定的腾讯墙内的数据显示到微擎墙内。</span>
			</div>
		</div>
		<div class="form-group" id="syncwall_tx" <?php  if(empty($reply['syncwall']['tx']['status'])) { ?>style="display:none;"<?php  } ?>>
			<label class="col-xs-12 col-sm-3 col-md-2 control-label">腾讯墙话题名</label>
			<div class="col-sm-9">
				<input type="text" value="<?php  echo $reply['syncwall']['tx']['subject'];?>" class="form-control" name="walls[tx][subject]">
				<span class="help-block"></span>
			</div>
		</div>
	</div>
